...
// path set to call the "jwtToken" service "email-exist" endpoint
func (c *Client) BuildEmailExistRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: EmailExistJWTTokenPath()}
	req, err := http.NewRequest("POST", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("jwtToken", "email-exist", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// EncodeEmailExistRequest returns an encoder for requests sent to the jwtToken
// email-exist server.
func EncodeEmailExistRequest(encoder func(*http.Request) goahttp.Encoder) func(*http.Request, interface{}) error {
	return func(req *http.Request, v interface{}) error {
		p, ok := v.(*jwttoken.EmailExistPayload)
		if !ok {
			return goahttp.ErrInvalidType("jwtToken", "email-exist", "*jwttoken.EmailExistPayload", v)
		}
		if p.Oauth != nil {
			head := *p.Oauth
			if !strings.Contains(head, " ") {
		...
		if err := encoder(req).Encode(&body); err != nil {
			return goahttp.ErrEncodingError("jwtToken", "email-exist", err)
		}
		return nil
	}
}
...
// jwtToken email-exist endpoint. restoreBody controls whether the response
...
func DecodeEmailExistResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("jwtToken", "email-exist", err)
			}
			err = ValidateEmailExistResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("jwtToken", "email-exist", err)
			}
		...
		case http.StatusBadRequest:
			...
			err = ValidateEmailExistEmailAlreadyExistResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("jwtToken", "email-exist", err)
			}
			return nil, NewEmailExistEmailAlreadyExist(&body)
		case http.StatusInternalServerError:
			var (
		...
		case http.StatusForbidden:
			var (
				body EmailExistInvalidScopesResponseBody
				err  error
			)
			err = decoder(resp).Decode(&body)
			if err != nil {
			...
			return nil, NewEmailExistInvalidScopes(body)
		case http.StatusUnauthorized:
			var (
				body EmailExistUnauthorizedResponseBody
				err  error
			)
			err = decoder(resp).Decode(&body)
			if err != nil {
			...
			return nil, NewEmailExistUnauthorized(body)
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("jwtToken", "email-exist", resp.StatusCode, string(body))
		}
	}
}
